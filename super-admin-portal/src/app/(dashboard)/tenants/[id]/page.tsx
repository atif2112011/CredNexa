  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <FormDialog title="Update tenant" triggerLabel="Update" endpoint={`/api/admin/tenants/${id}`} method="PATCH" fields={tenantUpdateFields} defaultValues={tenant} variant="outline" />
            <FormDialog title="Change status" triggerLabel="Activate / Deactivate" endpoint={`/api/admin/tenants/${id}/status`} method="PATCH" fields={statusFields} defaultValues={{ isActive: tenant.isActive ? "true" : "false" }} variant="secondary" />
          </>
        }
      />
      <div className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <JsonPanel title="Tenant Policy" data={data.tenantPolicy} />
          <JsonPanel title="Device Policies" data={data.devicePolicies} />
        </div>
        <Card>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <JsonPanel title="Device Summary" data={data.deviceSummary} />
          <JsonPanel title="Open Cases" data={data.openCases} />
        </div>
      </div>
    </>
  );
