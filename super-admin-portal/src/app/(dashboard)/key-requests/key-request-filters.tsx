...
export function KeyRequestFilters({ tenants, partners }: KeyRequestFiltersProps) {
    ...
    <div className="flex flex-wrap items-center gap-3">
      <form className="flex min-w-64 items-center gap-2" onSubmit={submitSearch}>
        <label className="relative block w-64">
          ...
          <span className="sr-only">Search key requests</span>
          <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Reference or status..." className="pl-9" />
        </label>
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        <select
          ...
          onChange={(event) => updateParam("tenantId", event.target.value)}
        ...
        >
          ...
          {tenants.map((tenant) => (
            <option key={String(tenant._id || tenant.id)} value={String(tenant._id || tenant.id)}>
