...
import { ArrowUpDown, CheckCircle2, ChevronLeft, ChevronRight, Loader2, Upload, XCircle } from "lucide-react";
...
function getPageHref(params: PayoutRequestsTableProps["searchParams"], page: number) {
  const nextParams = new URLSearchParams();
  Object.entries(params).forEach(([key, value]) => {
    if (value) nextParams.set(key, value);
  });
  nextParams.set("page", String(page));
  return `/payout-requests?${nextParams.toString()}`;
}

function getSortHref(params: PayoutRequestsTableProps["searchParams"], sortBy: string) {
  const nextParams = new URLSearchParams();
  Object.entries(params).forEach(([key, value]) => {
...
function buildDetailRows(detail: DetailPayload): [string, unknown, "badge" | "money" | "date" | "text"][] {
  const request = detail.payoutRequest || {};
  return [
    ["Partner", getName(request.channelPartnerId), "text"],
    ["Status", request.status, "badge"],
    ["Payout Amount", request.amount, "money"],
    ["UPI ID", request.upiId || "-", "text"],
    ["UPI Name", request.upiName || "-", "text"],
    ["Requested By", getName(request.requestedBy), "text"],
    ["Requested At", request.requestedAt || request.createdAt, "date"],
    ["Admin Reference Number", request.adminReferenceId || "-", "text"],
...
function validateProofFile(file: File | null) {
  if (!file) return "Payment proof image is required";
  if (!ALLOWED_PROOF_TYPES.has(file.type)) return "Payment proof must be JPEG, PNG, or WebP";
  if (file.size > MAX_PROOF_BYTES) return "Payment proof must be 5 MB or smaller";
  return "";
}
...
const tableColumns: SortableColumn[] = [
  ...
  { label: "Payout Amount", sortBy: "amount" },
  { label: "Creation Date", sortBy: "createdAt" },
  { label: "Approve" }
];

export function PayoutRequestsTable({ items, pagination, searchParams }: PayoutRequestsTableProps) {
  ...
  async function openDetail(request: RecordItem) {
    const payoutId = getId(request);
    setIsDetailOpen(true);
    setDetail(null);
    setDetailError("");
    setIsLoadingDetail(true);
    try {
      const response = await fetch(`/api/admin/partner-payouts/${payoutId}`);
      if (!response.ok) throw new Error(await getApiError(response));
      const result = (await response.json()) as { data: DetailPayload };
      setDetail(result.data);
    } catch (error) {
      setDetailError(error instanceof Error ? error.message : "Unable to load payout request details");
    } finally {
  ...
  async function submitAction() {
    if (!action) return;
    const payoutId = getId(action.request);
    ...
    if (action.type === "reject") {
      ...
      setActionError("");
      try {
        const response = await fetch(`/api/admin/partner-payouts/${payoutId}/reject`, {
          method: "POST",
        ...
        setAction(null);
        router.refresh();
        if (isDetailOpen) await openDetail(currentAction.request);
      } catch (error) {
    ...
    const proofError = validateProofFile(proofImage);
    if (proofError) {
      setActionError(proofError);
      return;
    }

    const body = new FormData();
    body.append("referenceId", trimmedReferenceId);
    body.append("proofImage", proofImage as File);

    setIsSubmittingAction(true);
    setActionError("");
    try {
      const response = await fetch(`/api/admin/partner-payouts/${payoutId}/approve`, {
        method: "POST",
        body
      });
      if (!response.ok) throw new Error(await getApiError(response));
  ...
  const detailRequest = detail?.payoutRequest;
    ...
    <>
        ...
        <CardContent className="p-0">
                ...
                {items.length ? (
                  items.map((request) => {
                    ...
                    return (
                      ...
                      >
                        <td className="max-w-64 truncate px-4 py-3.5 font-medium">{getName(request.channelPartnerId)}</td>
                        <td className="px-4 py-3.5">
                          <StatusBadge value={request.status} />
                        </td>
                        <td className="max-w-56 truncate px-4 py-3.5">{String(request.adminReferenceId || "-")}</td>
                        <td className="px-4 py-3.5 font-semibold text-emerald-700">{getAmount(request.amount)}</td>
          ...
          <div className="flex flex-col gap-3 border-t bg-muted/20 p-4 md:flex-row md:items-center md:justify-between">
            <p className="text-sm text-muted-foreground">
              Page {pagination.page} of {pagination.pages || 1} · {pagination.total} payout requests
            </p>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Review the selected partner payout request.</DialogDescription>
          </DialogHeader>
          {isLoadingDetail ? (
      ...
      <Dialog open={Boolean(action)} onOpenChange={(open) => !open && setAction(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{action?.type === "approve" ? "Approve Payout Request" : "Reject Payout Request"}</DialogTitle>
            <DialogDescription>
              {action?.type === "approve"
                ? "Enter the admin payment reference and upload payment proof before approving."
                : "Add a rejection note before rejecting this payout request."}
            </DialogDescription>
          </DialogHeader>
          {action ? (
            <div className="space-y-4">
              <div className="rounded-lg border bg-muted/30 p-3 text-sm">
                <div className="font-medium">{getName(action.request.channelPartnerId)}</div>
                <div className="text-muted-foreground">{getAmount(action.request.amount)}</div>
              </div>
              {action.type === "approve" ? (
                <>
                  <div className="grid gap-2">
                    <Label htmlFor="referenceId">Reference Number</Label>
                    <Input id="referenceId" value={referenceId} onChange={(event) => setReferenceId(event.target.value)} placeholder="UTR123456" />
                  </div>
                  <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="actionNote">Rejection Note</Label>
                  <Textarea
                    id="actionNote"
                    value={actionNote}
                    onChange={(event) => setActionNote(event.target.value)}
                    placeholder="UPI details or payment verification failed"
                  />
                </div>
              )}
              {actionError ? (
          ...
          <DialogFooter>
            ...
            <Button type="button" variant={action?.type === "reject" ? "destructive" : "default"} onClick={submitAction} disabled={isSubmittingAction}>
              {isSubmittingAction ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> : action?.type === "approve" ? <Upload className="h-4 w-4" aria-hidden="true" /> : null}
      ...
      <ImageViewerModal
        ...
        title="Payment Proof"
        description="Partner payout payment proof"
      />
    </>
  );
}
